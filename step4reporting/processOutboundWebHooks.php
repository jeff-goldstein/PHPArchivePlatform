...
$loggingFlag = $config['processing']['LoggingFlag'];
$top = "msys";
	...
	if (!strpos($currentFile, ".lck"))
	{
		...
		if ($success)
		{
			...
			foreach ($arrayContent as $item)
			{
				...
				$event_time = date("Y-m-d H:i:s", $item[$top][$event_key]["timestamp"]);
				...
				if ($store)
				{
    				...
    				{
        				$sql = "INSERT INTO austein_archiver.events (campaign_id, friendly_from, injection_time, rcpt_to, rcpt_type, subject, UID, event_type, event_time, raw) VALUES ('" . $campaign_id . "', '" . $friendly_from . "', '" . $injection_time . "', '" . $rcpt_to . "', '" . $rcpt_type . "', '" . $subject . "', '" . $uid . "', '" . $event_type . "', '" . $event_time . "', '" . $raw . "')";
